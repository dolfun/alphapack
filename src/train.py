...
def train_policy_value_network(model, episodes, device, config):
  ...
  for epoch in range(config.epochs):
    ...
    for inputs in tqdm(dataloader, leave=False):
      ...
      train_log_file.write(f',{total_loss.item()},{priors_loss.item()},{value_pdf_loss.item()},{value_cdf_loss.item()}\n')
    ...
    print(f'Loss: {epoch_loss:.4f} => {epoch_priors_loss:.4f} + {epoch_value_pdf_loss:.4f} + {epoch_value_cdf_loss:.4f}')